...
class Polygon extends Shape {
  ...
  public Point[] getPoints() {
    ...
    for (int i = 0; i < shape.length; i++) {
      double x = ((shape[i].x-center.x) * Math.cos(Math.toRadians(rotation)))
              - ((shape[i].y-center.y) * Math.sin(Math.toRadians(rotation)))
              + center.x + position.x;
      double y = ((shape[i].x-center.x) * Math.sin(Math.toRadians(rotation)))
              + ((shape[i].y-center.y) * Math.cos(Math.toRadians(rotation)))
              + center.y + position.y;
